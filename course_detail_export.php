...
<head>
    ...
    <style>
        ...
        .title { text-align: center; font-weight: bold; font-size: 15pt; margin-bottom: 18px; text-transform: uppercase; }
        .indent { text-indent: 28px; }
        .center { text-align: center; }
    </style>
</head>
<body>
    <div class="title">Đề cương chi tiết học phần<br><?= pdf_text(mb_strtoupper($module['name'], 'UTF-8')) ?></div>

    <h1>1. Thông tin học phần</h1>
    <table class="info">
        <tr><td>Học phần nền: <?= pdf_text($module['course_name'] ?? '-') ?></td></tr>
        <tr><td>Mã học phần: <?= pdf_text($module['code']) ?></td></tr>
        <tr><td>Tính chất học phần: <?= pdf_text($module['type']) ?></td></tr>
        <tr><td>Tổng tín chỉ: <?= pdf_text((string)$module['credits']) ?> (LT <?= pdf_text((string)$module['credits_theory']) ?> / TH <?= pdf_text((string)$module['credits_practice']) ?>)</td></tr>
        <tr><td>Phân bổ thời gian: <?= pdf_text((string)$module['total_hours']) ?> (LT <?= pdf_text((string)$module['theory_hours']) ?> / TH <?= pdf_text((string)$module['practical_hours']) ?>)</td></tr>
        <tr><td>Số giờ tự học: <?= pdf_text((string)$module['self_study_hours']) ?></td></tr>
        <tr><td>Đối tượng người học: <?= pdf_text($module['target_programs']) ?></td></tr>
        <tr><td>Học kỳ / Năm học: <?= pdf_text($module['expected_semester']) ?> / <?= pdf_text($module['expected_year']) ?></td></tr>
        <tr><td>Học phần tiên quyết: <?= pdf_text($module['prerequisite_text'] ?: 'Không') ?></td></tr>
        <tr><td>Học phần song hành: <?= pdf_text($module['parallel_text'] ?: 'Không') ?></td></tr>
        <tr><td>Học phần học trước: <?= pdf_text($module['previous_text'] ?: 'Không') ?></td></tr>
        <tr><td>Bộ môn tham gia: <?= pdf_text($module['department_in_charge_text'] ?: 'Không') ?></td></tr>
        <tr><td>Ban điều phối: <?= pdf_text(implode(', ', $module['coordinator_names'] ?? []) ?: ($module['coordinating_board'] ?? '')) ?></td></tr>
        <tr><td>Khoa phụ trách: <?= pdf_text($module['faculty_in_charge']) ?></td></tr>
        <tr><td>Hình thức giảng dạy chung: <?= pdf_text($module['delivery_mode']) ?></td></tr>
    </table>

    <h1>2. Mô tả học phần</h1>
    <p class="indent"><?= nl2br(pdf_text($module['description'])) ?></p>

    <h1>3. Mục tiêu và chuẩn đầu ra học phần</h1>
    <h2>3.1. Mục tiêu học phần</h2>
    <p class="indent"><?= nl2br(pdf_text($module['objectives'])) ?></p>
    <h2>3.2. CLO</h2>
    <table>
        <thead>
            <tr>
        ...
        <tbody>
            <?php if (!empty($clos)): ?>
                <?php foreach ($clos as $clo): ?>
                    <tr>
                        <td class="center"><?= pdf_text($clo['code']) ?></td>
                        <td><?= pdf_text($clo['domain']) ?></td>
                        <td><?= pdf_text($clo['bloom_level']) ?></td>
                        <td><?= pdf_text($clo['contribution_level']) ?></td>
                        <td><?= pdf_text($clo['pi_id']) ?></td>
                        <td><?= pdf_text($clo['plo_pi']) ?></td>
                        <td><?= nl2br(pdf_text($clo['content'])) ?></td>
                    </tr>
                <?php endforeach; ?>
            <?php else: ?>
    ...
    <p class="indent"><?= nl2br(pdf_text($module['grading_scale'])) ?></p>
        ...
        <tbody>
            <?php if (!empty($assessments)): ?>
                <?php foreach ($assessments as $assessment): ?>
                    <tr>
                        <td><?= pdf_text($assessment['assessment_category'] ?: $assessment['type']) ?></td>
                        <td><?= pdf_text($assessment['clos_codes'] ?: $assessment['clos_text']) ?></td>
                        <td><?= pdf_text($assessment['plo_pi']) ?></td>
                        <td><?= pdf_text($assessment['tool_names'] ?: $assessment['tool']) ?></td>
                        <td class="center"><?= pdf_text((string)$assessment['weight']) ?>%</td>
                    </tr>
                <?php endforeach; ?>
            <?php else: ?>
        ...
        <tbody>
            <?php if (!empty($selfStudyActivities)): ?>
                <?php foreach ($selfStudyActivities as $item): ?>
                    <tr>
                        <td><?= pdf_text($item['activity_name']) ?></td>
                        <td><?= pdf_text($item['clos_codes'] ?: $item['clos_text']) ?></td>
                        <td class="center"><?= pdf_text((string)$item['duration_hours']) ?></td>
                        <td><?= pdf_text($item['method']) ?></td>
                        <td><?= pdf_text($item['assessment_method']) ?></td>
                        <td><?= pdf_text($item['evidence']) ?></td>
                    </tr>
                <?php endforeach; ?>
            <?php else: ?>
        ...
        <tbody>
            <?php if (!empty($theoryTopics)): ?>
                <?php foreach ($theoryTopics as $topic): ?>
                    <tr>
                        <td><?= pdf_text($topic['chapter']) ?></td>
                        <td><?= pdf_text($topic['parent_title']) ?></td>
                        <td><?= pdf_text($topic['title']) ?></td>
                        <td><?= pdf_text($topic['method']) ?></td>
                        <td><?= pdf_text($topic['teaching_method']) ?></td>
                        <td class="center"><?= pdf_text((string)$topic['class_hours']) ?></td>
                        <td class="center"><?= pdf_text((string)$topic['online_hours']) ?></td>
                        <td class="center"><?= pdf_text((string)$topic['self_study_hours']) ?></td>
                        <td><?= pdf_text($topic['clos_codes'] ?: $topic['clos_text']) ?></td>
                    </tr>
                <?php endforeach; ?>
            <?php else: ?>
        ...
        <tbody>
            <?php if (!empty($practicalTopics)): ?>
                <?php foreach ($practicalTopics as $topic): ?>
                    <tr>
                        <td><?= pdf_text($topic['topic']) ?></td>
                        <td><?= pdf_text($topic['content']) ?></td>
                        <td><?= pdf_text($topic['method']) ?></td>
                        <td><?= pdf_text($topic['teaching_method']) ?></td>
                        <td class="center"><?= pdf_text((string)$topic['lab_hours']) ?></td>
                        <td class="center"><?= pdf_text((string)$topic['online_hours']) ?></td>
                        <td><?= pdf_text($topic['clos_codes'] ?: $topic['clos_text']) ?></td>
                        <td><?= pdf_text($topic['facility_name']) ?></td>
                    </tr>
                <?php endforeach; ?>
            <?php else: ?>
            ...
            <?php if (!empty($combinedTopics)): ?>
                <?php foreach ($combinedTopics as $index => $topic): ?>
                    <tr>
                        ...
                        <td><?= pdf_text($topic['method']) ?></td>
                        <td><?= pdf_text($topic['teaching_method']) ?></td>
                        <td class="center"><?= pdf_text((string)$topic['theory_hours']) ?></td>
                        <td class="center"><?= pdf_text((string)$topic['practical_hours']) ?></td>
                        <td class="center"><?= pdf_text((string)$topic['online_hours']) ?></td>
                        <td class="center"><?= pdf_text((string)$topic['self_study_hours']) ?></td>
                        <td><?= pdf_text($topic['clos_codes'] ?: $topic['clos_text']) ?></td>
                        <td><?= pdf_text($topic['facility_name']) ?></td>
        ...
        <tbody>
            <?php if (!empty($teachResources)): ?>
                <?php foreach ($teachResources as $index => $resource): ?>
                    <tr>
                        <td class="center"><?= $index + 1 ?></td>
                        <td><?= pdf_text($resource['title']) ?></td>
                        <td><?= pdf_text($resource['editor']) ?></td>
                        <td><?= pdf_text($resource['publisher']) ?></td>
                        <td class="center"><?= pdf_text($resource['year']) ?></td>
                        <td><?= pdf_text($resource['identifier']) ?></td>
                    </tr>
                <?php endforeach; ?>
            <?php else: ?>
...
$safeCode = preg_replace('/[^A-Za-z0-9_\-]/', '_', (string)$module['code']);
...
$mpdf->SetTitle('Đề cương học phần ' . ($module['name'] ?? ''));
$mpdf->WriteHTML($html);
